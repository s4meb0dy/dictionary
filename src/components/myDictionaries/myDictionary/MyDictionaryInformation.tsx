...
const DictionaryInformation: React.FC<DictionaryInformationProps> = ({
    ...
    const { totalInformationAboutMyDictionary, idOfWordsToStudy } =
    ...
    React.useEffect(() => {
        if (
            idOfWordsToStudy.length === 0 ||
            idOfWordsToStudy.length ===
                totalInformationAboutMyDictionary.totalWords
        )
    ...
    }, [idOfWordsToStudy, totalInformationAboutMyDictionary])
            ...
            {!isLoading ? (
                <>
                    ...
                    <p className="text-white tracking-wide backdrop:text-[20px] leading-[25px]">
                        {`${totalInformationAboutMyDictionary.totalWords} words`}
                    </p>
                    <p className="text-white tracking-wide backdrop:text-[18px] leading-[23px]">
                        {`${totalInformationAboutMyDictionary.totalLearnedWords} studied words`}
                    </p>
                </>
            ) : (
                <ContentLoader
